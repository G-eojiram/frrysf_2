            ...
            <DialogPanel
              ...
              <div class="sm:flex sm:items-start">
                <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                  ...
                  <div class="overflow-hidden rounded-lg border border-gray-300 shadow-sm focus-within:border-teal-300 focus-within:ring-1 focus-within:ring-indigo-500">
                      <div>
                      ...
                      <div class="flex sm:text-[12px] md:text-[13px] px-[1rem] gap-x-2 border-t py-2">
                        <div class="flex items-center gap-x-1">
                          <label for="start-datetime" class="text-gray-400">Event Start:</label>
                          <p class="sm:text-[9.4px] md:text-[12px]">{{ startDateTime }}</p>
                          <input type="datetime-local" id="start-datetime" name="start-datetime" value="" ref="startDateInput" @input="handleStartDateTimeInput"
                        ...
                        <div class="flex items-center gap-x-1">
                          ...
                          <input type="datetime-local" id="end-datetime" name="end-datetime" value="" ref="endDateInput" @input="handleEndDateTimeInput"
                            class="w-[1rem] h-[1rem] text-transparent bg-transparent border-none cursor-pointer focus:outline-none" />
                          </div>
                      </div>
                      <div class="mt-1 border-t border-gray-200">
                          <label for="caption" class="sr-only">Event Caption</label>
                          <textarea v-model="caption" name="caption" placeholder="Write a caption or description of this Event..."
                          ...
                          <div>
                            <router-link>
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"  width="25" height="25">
                                <path fill="#f03d3d" d="M172.3 501.7C27 291 0 269.4 0 192 0 86 86 0 192 0s192 86 192 192c0 77.4-27 99-172.3 309.7-9.5 13.8-29.9 13.8-39.5 0zM192 272c44.2 0 80-35.8 80-80s-35.8-80-80-80-80 35.8-80 80 35.8 80 80 80z"/>
                              </svg>
                            </router-link>
                          </div>
                        </div>
                  ...
                  <div class="px-[2rem] py-[1rem]">
                    ...
                    <div v-if="imageUrls.length > 0" class="grid grid-cols-4 gap-2">
                      <div v-for="(imageUrl, index) in imageUrls" :key="index" class="relative">
                        <img :src="imageUrl" alt="Uploaded Image" class="max-w-[100px] max-h-[100px] object-contain border"/>
                        <button @click="removeImage(index)" class="absolute top-0 right-0 p-1 text-gray-500 hover:text-gray-800">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                          </svg>
                        </button>
