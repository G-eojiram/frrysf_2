<template>
    <div>
        <div id="marginright" class="pb-[2rem] lg:mx-[3rem] flex justify-between items-center">
            <div>
                <router-link to="/animalprofile" class="flex items-center">
            ...
            <div id="switchcontainer" class="flex items-center gap-2">
                <button id="switch1" class="flex items-center gap-x-2 py-2 px-[2rem] sm:px-[1rem] rounded-md hover:bg-bgteal bgteal sm:text-[13.5px] md:text-[14px] text-white">
                ...
                <router-link to="/editAnimalProfileform">
                    <button id="switch2" type="button"
                        class="inline-flex justify-center rounded-md bg-white sm:px-[2rem]  md:px-[2rem] lg:px-[13px]  py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
        ...
        <div id="margin" class="sm:mx-[1rem] md:mx-[10rem]">
            <div class="">
                ...
                <div>
                    <p class="mt-1 max-w-2xl text-sm leading-6 text-gray-500">This Information related to this Furry Animal will be displayed publicly.</p>
                </div>
            </div>
            <div class="mt-6">
                <dl class="grid grid-cols-1 sm:grid-cols-2">
                    <div class="border-t border-gray-100 px-4 py-6 sm:col-span-1 sm:px-0">
                        ...
                        <dd class="mt-1 text-sm leading-6 text-gray-700 sm:mt-2">{{ `${info.name} "${info.nickname}"` }}</dd>
                    </div>
                    <div class="border-t border-gray-100 px-4 py-6 sm:col-span-1 sm:px-0">
                        <dt class="text-sm font-medium leading-6 text-gray-900">Age / Gender</dt>
                        <dd class="mt-1 text-sm leading-6 text-gray-700 sm:mt-2">{{ `${info.age}, ${info.gender}` }}</dd>
                    </div>
                    <div class="border-t border-gray-100 px-4 py-6 sm:col-span-1 sm:px-0">
                    ...
                    <div class="border-t border-gray-100 px-4 py-6 sm:col-span-2 sm:px-0">
                        ...
                        <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0" v-for="(item, index) in healthAndMedical" :key="index">
                        <dt class="text-sm font-normal  leading-6 text-gray-900 mr-[2rem]">{{ item.label }}</dt>
                        <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{ item.status }} {{ item.details ? `(${item.details})` : '' }}</dd>
                        </div>
                    </div>
